...
class PdoHelper
{
     ...
     * @return int
     */
    public function sum($table, $k, $where = null)
    {
        $conditions = $this->_where($where);
        $count = $this->getColumn("SELECT SUM(`" . $k . "`) FROM pre_" . $table . $conditions["_where"], $conditions["_bindParams"]);
        return intval($count);
    }
